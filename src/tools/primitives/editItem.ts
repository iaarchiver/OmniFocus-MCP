...
`;
  ...
  if (id) {
    if (itemType === 'task') {
      script += `
      -- Try to find task by ID using whose clause
      try
        set foundItem to first flattened task whose id = "${id}"
      end try

      -- If not found in projects, search in inbox
      if foundItem is missing value then
        try
          set foundItem to first inbox task whose id = "${id}"
        end try
      end if
`;
...
`;
  ...
  if (!id && name) {
    if (itemType === 'task') {
      script += `
      -- Find task by name using whose clause
      try
        set foundItem to first flattened task whose name = "${name}"
      end try

      -- If not found in projects, search in inbox
      if foundItem is missing value then
        try
          set foundItem to first inbox task whose name = "${name}"
        end try
      end if
`;
    } else if (itemType === 'project') {
